
const VGA = @as(*volatile [80 * 25]u16, @ptrFromInt(0xB8000));
const enable_pae = @import("paging/enable_pae.zig");

// Provided by linker.ld so we can zero .bss ourselves (objcopy strips it).
extern var __bss_start: u8;
extern var __bss_end: u8;

// zero .bss so Zig globals stay predictable
fn bss_clear() void {
	const start = @intFromPtr(&__bss_start);
	const end = @intFromPtr(&__bss_end);
	var p: usize = start;
	while (p < end) : (p += 1) {
		@as(*volatile u8, @ptrFromInt(p)).* = 0;
	}
}

// clear to gray-on-black
fn vga_clear(attr: u8) void {
	var i: usize = 0;
	while (i < 80 * 25) : (i += 1) {
		// high byte = attr, low byte = ' '
		VGA[i] = (@as(u16, attr) << 8) | ' ';
	}
}

// prints in vga (0x0f for white color)
fn vga_print(msg: []const u8, row: u8, col: u8, attr: u8) void {
	var i: usize = 0;
	var pos: usize = (@as(usize, row) * 80 + @as(usize, col));
	while (i < msg.len and pos < 80 * 25) : ({ i += 1; pos += 1; }) {
		const ch: u8 = msg[i];
		VGA[pos] = (@as(u16, attr) << 8) | ch;
	}
}

pub export fn _start() linksection(".text.start") callconv(.c) noreturn {

	vga_clear(0x07);

	bss_clear();

	vga_print("hello from 32-bit protected mode", 0, 0, 0x0F);
	vga_print("long-mode stage active", 1, 0, 0x0E);
	enable_pae.prepare_long_mode_paging();

	// hang forever (hlt in a loop)
	while (true) {
		asm volatile ("hlt");
	}
}
